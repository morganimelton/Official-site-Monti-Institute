...
function home() {
  ...
  <section class="hero-home reveal" data-media="hero-media" data-video-ready="true">
    ...
    <div class="wrap hero-home__inner hero-home__layout">
      ...
      <h1>Where neurology<br><em>meets aesthetics.</em><br>Care that is<br>personal by design.</h1>
  ...
  <section class="duality reveal" id="care"><div class="duality__split"><article class="duality-panel duality-panel--neuro">${mediaSlot("media-neurology","Neurological medicine media slot")}<div><p class="kicker">Neurology</p><h3>Advanced care for complex neurological conditions.</h3><ul>${["Migraine & Headache","Neurological Pain","Diagnostic Neurology","Advanced Therapies"].map(li).join("")}</ul><a class="text-link" href="neurological-medicine.html">Explore Neurology</a></div></article><span class="duality__mark">${mark()}</span><article class="duality-panel duality-panel--aesthetic">${mediaSlot("media-aesthetics","Aesthetic medicine media slot")}<div><p class="kicker">Aesthetics</p><h3>Refined aesthetics.<br>Natural results.</h3><ul>${["Facial Balance","Injectables","Regenerative Aesthetics","Energy-Based Treatments"].map(li).join("")}</ul><a class="text-link" href="aesthetic-medicine.html">Explore Aesthetics</a></div></article></div></section>
  <section class="difference reveal"><div class="wrap difference__inner"><div><p class="kicker">The Monti Difference</p><h2>Precision<br>over volume.</h2></div><p class="difference__statement">Longer appointments, individualized planning, clinical continuity, and physician-led decisions keep care focused and considered.</p><div class="difference__list">${["Longer Appointments","Individualized Evaluation","Clinical Continuity","Physician-Led Decisions"].map((x,i)=>`<article>${approachIcon(["Anatomy","Evidence","Precision","Restraint"][i])}<h3>${x}</h3></article>`).join("")}</div></div></section>
  ...
  <section class="institute-experience reveal" data-video-ready="true"><div class="wrap"><div class="institute-experience__media">${mediaSlot("media-patient-experience","Institute experience media slot")}</div><div class="institute-experience__panel"><p class="kicker">The Institute</p><h2>Designed around the way care should feel.</h2><p>Monti's current philosophy emphasizes longer appointments, individualized planning, continuity, thoughtful care, and a private clinical environment.</p><div class="institute-experience__attributes"><span>Private</span><span>Considered</span><span>Unhurried</span><span>Individualized</span></div><a class="text-link" href="the-institute.html">Visit the Institute</a></div></div></section>
  <section class="bottom-utility reveal"><div class="wrap bottom-utility__grid"><article class="bottom-utility__panel concierge-cta"><span class="utility-graphic" aria-hidden="true"></span><p class="kicker">Digital Concierge</p><h2>Not sure where to begin?</h2><p>We'll help you find the right care.</p><button class="text-link" type="button" data-open-concierge>Find My Care Path</button></article><article class="bottom-utility__panel shop-preview"><div>${mediaSlot("media-shop-product","Product media slot")}</div><p class="kicker">The Monti Edit</p><h2>Selected for continued care.</h2><p>Curated partner dispensaries that extend appropriate care beyond the Institute.</p><a class="text-link" href="shop.html">Visit the Shop</a></article><article class="bottom-utility__panel access"><span class="access-watermark" aria-hidden="true">${mark()}</span><p class="kicker">For Current Patients</p><nav>${[["Patient Portal",CONFIG.bookingUrl],["Insurance & Payment","insurance-payment.html"],["Financing Options","insurance-payment.html"],["Contact Us","contact.html"]].map(([t,h])=>`<a href="${h}">${t}</a>`).join("")}</nav></article></div></section>
  `;
}

function approachItem(title, text) {
  return `<article>${approachIcon(title)}<h3>${title}</h3><p>${text}</p></article>`;
}

function approachIcon(title) {
  ...
  return `<svg viewBox="0 0 56 56" aria-hidden="true"><path d="${icon}"/></svg>`;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("[data-infusion-tab]").forEach(button => {
    button.addEventListener("click", () => setInfusionTab(button));
    button.addEventListener("keydown", event => {
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        setInfusionTab(button);
      }
    });
  });
